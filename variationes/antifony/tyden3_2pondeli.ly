...
\score {
  \relative c' {
    \choralniRezim
    f4 f e f d d \barMin
    \mark\sipka e f d( c) c \barMaior
    a c d d( f) f \mark\sipka
    f e c e f d d \barFinalis
...
\score {
  \relative c' {
    \choralniRezim
    f4( g a) f g \barMaior
    g f g a a \mark\sipka f g( f) d \barMin
    f g g f \barFinalis
  }
  \addlyrics {
    Bla -- ze těm,
...
\score {
  \relative c' {
    \choralniRezim
    f4 g a f g f d d \barMaior
    \mark\sipka c d( f) f \barMin g a g( f) f \barFinalis
  }
  \addlyrics {
    Bla -- ze těm, kdo pře -- bý -- va -- jí
    v_tvém do -- mě, Hos -- po -- di -- ne.
...
\score {
  \relative c' {
    ...
    d c( d) e f e e \barMin
    f4( a) g f( e) e \barMaior
    a g a \mark\sipka b b a g a( f e) e \barFinalis
  }
  \addlyrics {
    Na -- vě -- ky ne -- za -- po -- me -- nu na tvá na -- ří -- ze -- ní,
    Hos -- po -- di -- ne,
...
\score {
  \relative c'' {
    \choralniRezim
    c4 d d e d c d d \barMin f e d e d d \barMin
    c b a( g) g \barMaior
...
\markup\justify{
  Závěr g-d-e není v repertoáru antifon oficia běžný, ale ani úplně
  neslýchaný. V rámci dat, která se mi zatím podařilo shromáždit
  v databázi Adiutor, se sporadicky objevuje v dominikánském antifonáři
  (a pak v invitatoriích svatopetrského Liber nocturnalis,
  což je ovšem pochybný zdroj s velkým množstvím
  neogregoriánských komposic).
}
\score {
  \relative c' {
    \zvyraznovacModry
...
\score {
  \relative c' {
    ...
    \mark\sipka g f e( f) d d c f e c( d) d \barFinalis
  }
  \addlyrics {
    Na -- še po -- moc je ve jmé -- nu Hos -- po -- di -- na,
    kte -- rý u -- či -- nil ne -- be a ze -- mi.
