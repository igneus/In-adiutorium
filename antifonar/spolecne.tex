...
\newcommand{\inadiutoriumWeb}{http://www.inadiutorium.cz}
...
\newcommand{\tirazSazbaProgramem}{
  Sazba programy \LaTeX\mbox{} a LilyPond (\lilypondversion)
}
...
\newcommand{\licencniPoznamka}{%
  ...
  Zjištěné odchylky od vydání breviáře z r. 1994 jsou
  podle něj opraveny.
}
...
\newcommand{\malaTiraz}{
  ...
  projekt: In adiutorium - noty k liturgii hodin \url{\inadiutoriumWeb}
...
\newcommand{\ziveZahlaviLicha}[1]{%
  \fancyhead[CO]{#1}}
\newcommand{\ziveZahlaviSuda}[1]{%
  \fancyhead[CE]{#1}}
\newcommand{\ziveZahlaviObe}[1]{%
  \fancyhead[CO,CE]{#1}}
...
% alternativa \dotfill pro index zalmu - ve stisnenych podminkach
% brani tomu, aby se vypln uplne preskocila, coz by v indexu zalmu
% znamenalo tesne sousedstvi cisel biblickeho odkazu a cisel
% odkazu na stranku, a tedy necitelny/nesmyslny vysledek
\newcommand{\idxDotfill}{%
...
\newcommand{\nadpisZalmuBezMezery}[1]{%
  ...
  \begin{center}%
    \textbf{#1}%
  \end{center}%
  \ifdef{\indexOfPsalmsEnabled}{\index[zalmy]{#1|hyperpage}}{}%
  \nopagebreak%
}

% \nadpisZalmu{Žalm 94}         % vytisteny titulek i do indexu zalmu
% \nadpisZalmu[Žalm 118-II]{II} % volitelny argument do indexu zalmu, povinny vytisknout
\newcommand{\nadpisZalmu}[2][]{%
  \vspace{2mm}%
  \begin{center}%
    \textbf{#2}%
  \end{center}%
  \ifdef{\indexOfPsalmsEnabled}{%
    \ifstrempty{#1}{\index[zalmy]{#2|hyperpage}}{\index[zalmy]{#1|hyperpage}}%
  }{}%
  \nopagebreak%
}

% argumenty jako \nadpisZalmu
% (standardni prikaz pouzivany programem `pslm`; variantu s nepovinnym argumentem
% pslm nezna, v pripade potreby ji generujeme zvlastnim krokem)
...
\newcommand{\psalmStrophe}{\hspace*{0pt}\hfill\znackaStrofaZalmu}
